...
- [ ] **VS-035B — Backend/Worker:** Durable publication-job progress. Mapping: `B-031`, `B-032`; tracking backend `#30`.
  ...
  - Partial checkpoint: default-off migration + transactional admission/outbox/read model cùng public-only UUID-keyset builder, measured feature/vertex progress, DB lease/heartbeat/recovery, redacted terminal failure về `approved`, final atomic snapshot/pointer và duplicate/crash no-op đã có real Postgres/Redis coverage. Guard-intact pretrial exact-SHA đã được independent audit chấp nhận, nhưng production-mode activation chưa chạy nên VS-035B vẫn Open.
  - Activation preparation: backend `b3e12df3f7c728bf2499fc7fb90ff3a65762f3c8` + frontend `6e6fe83f7dbf6d5a01c710bb35e670e08b63e1b8` pass hai fresh-volume run ở `development` với checksummed evidence ghi trong activation runbook. Guard-removal đang là WIP riêng, mọi default vẫn false; independent review và hai run production-mode vẫn pending, mục này/backend #30 tiếp tục Open/NO-GO.
