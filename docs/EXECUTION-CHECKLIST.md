...
- [ ] **VS-035B — Backend/Worker:** Durable publication-job progress. Mapping: `B-031`, `B-032`; tracking backend `#30`.
  ...
  - Partial checkpoint: default-off migration + transactional admission/outbox/read model cùng public-only UUID-keyset builder, measured feature/vertex progress, DB lease/heartbeat/recovery, redacted terminal failure về `approved`, final atomic snapshot/pointer và duplicate/crash no-op đã có real Postgres/Redis coverage. Production config vẫn fail-closed; frontend polling/exact full-stack activation chưa được chấp nhận nên VS-035B vẫn Open.
  - Activation preparation: nhánh `test/durable-publication-activation` thêm fixture generation-one + approved 3-feature successor, split filesystem phase protocol, real Nest worker `SIGKILL`/lease recovery, 6-file HTTPS gate và SHA-256 evidence manifest. Frontend activation SHA `6e6fe83f7dbf6d5a01c710bb35e670e08b63e1b8` đã được pin/review; independent backend review và two-run fresh-volume evidence vẫn pending. Production guard chưa gỡ và mục này vẫn Open.
