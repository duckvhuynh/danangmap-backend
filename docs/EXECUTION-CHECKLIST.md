...
- [ ] **VS-035B — Backend/Worker:** Durable publication-job progress. Mapping: `B-031`, `B-032`; tracking backend `#30`.
  ...
  - Partial checkpoint: default-off migration + transactional queued admission + outbox lease/fair reconciliation + deterministic Bull job ID + bounded/redacted ETag/cursor read model đã có. Production config fail-closed khi cờ bật. Chưa có builder/processor, measured build progress, terminal failure recovery hoặc pointer switch; VS-035B vẫn Open.
