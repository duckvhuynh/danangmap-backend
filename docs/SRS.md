...
| WFL-012 | Checkpoint default-off của durable publication có transactional queued job + outbox, deterministic Bull dispatch/reconciliation, UUID-keyset batch checkpoint chỉ chứa public allowlist, measured feature progress, lease/heartbeat recovery, failure an toàn về `approved`, và final pointer switch nguyên tử/idempotent. Khi bật explicit, kể cả production, admission bắt buộc `clientIntent=desktop`; mọi default vẫn false. Release NO-GO tới guard-removal review + production-mode two-run evidence phải pass; backend #30/frontend #19 tiếp tục Open/In Progress. |
