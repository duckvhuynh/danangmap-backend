...
| B-064 | Audit/workflow/revision/publication history query endpoints | `danangmap-backend` | B-014, B-016 | Chín canonical endpoints; bounded feature-level diff cursor + circle radius/redaction; typed attachment add/remove/reorder với private redaction; history/pointer ETag riêng; global audit System Admin-only, content role scope immutable |
...
| Q-027 | Audit/history scope-redaction integration test | `danangmap-backend` | B-064 | Nine path/OpenAPI assertions; stable diff/audit cursor; System Admin global, content-role scoped, immutable redacted metadata; 25k/2m bound; typed attachment diff + private redaction; generated OpenAPI artifact |
