...
| Auth admin                | Tài khoản nội bộ; session opaque trong cookie HttpOnly; MFA tùy chọn qua env, mặc định tắt        |
...
System Admin kế thừa quyền Editor/Reviewer/Publisher. Việc kế thừa chỉ mở capability; state machine, CSRF, ETag, idempotency và separation-of-duties vẫn áp dụng như với vai trò nội dung tương ứng. Việc đổi vai trò phải thu hồi toàn bộ session đang hoạt động và được audit.
...
| AUTH-004 | `MFA_ENABLED` mặc định `false`. Khi `true`, MFA TOTP là bắt buộc và user mới chỉ có pre-auth session cho đến khi enroll/xác minh; khi `false`, password hợp lệ cấp authenticated session trực tiếp. Tắt policy không xóa factor hoặc recovery code đã lưu.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| AUTH-010 | Invite có endpoint inspect an toàn và accept một lần; accept đặt password, sau đó enroll MFA trước admin session khi policy bật hoặc nhận authenticated session trực tiếp khi policy tắt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| AUTH-011 | Password reset request PHẢI trả generic `202` với timing tương đương cho account có/không tồn tại, có durable idempotency/rate limit; completed replay cùng key/body phải sống qua API restart, không tiêu thụ thêm rate-limit slot hoặc tạo thêm outbox. Token random một lần chỉ lưu hash, được copy/paste và chỉ nhận trong body (không URL/browser storage/log); confirm nguyên tử revoke mọi authenticated/pre-auth session, pending challenge và reset token, rồi yêu cầu login lại theo policy MFA hiện hành.                                                                                                                                                                              |
...
| AUTH-015 | System Admin có directory user/invite phân trang, search/filter và user detail an toàn gồm summary MFA/session/invite/reset. Không response/audit/log nào chứa password/session hash, MFA secret, recovery code/digest, invite/reset token hoặc encrypted mail payload.                                                                                                                                                                                                                                                                                                                                                                                                                           |
| AUTH-016 | Mọi admin identity mutation dùng CSRF + Origin, idempotency và strong ETag. Đổi role/status/MFA revoke session nguyên tử; last-active-System-Admin được bảo vệ; resend invite vô hiệu credential cũ; admin reset password chỉ enqueue mail và không trả token. Security mutation tự nhắm actor dùng self-service flow thay vì admin override.                                                                                                                                                                                                                                                                                                                                                     |
| AUTH-017 | Database mới không ship default credential/production seed. Bootstrap chỉ available khi operator cấu hình token high-entropy và `users` rỗng; POST yêu cầu rate limit, Origin/CSRF và constant-time token validation. Transaction + PostgreSQL advisory lock cho đúng một System Admin thắng, trả pre-auth MFA enrollment khi policy bật hoặc authenticated session khi tắt; concurrent/replay fail closed. Password/token/TOTP secret/recovery code không xuất hiện trong response, log hoặc audit metadata.                                                                                                                                                                                     |
...
| WFL-004 | Separation-of-duties được kiểm tra ở mọi transition và rollback bằng participant history bất biến. Create/update/delete feature và import apply ghi `edit`; actor từng edit/review vẫn bị deny publish/rollback sau khi đổi role; System Admin có đầy đủ capability nhưng không bypass participant history.                                                                                                                                                                                                                                                                                       |
...
| WFL-011 | Attachment diff dùng canonical versioned association, trả summary và feature entry typed cho add/remove/reorder của field public non-sensitive. Association private/sensitive chỉ trả count/boolean redacted; không lộ descriptor, object key, checksum, owner hoặc suy diễn từ JSON properties. Query aggregate/page là set-based và bounded theo feature page.                                                                                                                                                                                                                                  |
| WFL-012 | Checkpoint default-off của durable publication có transactional queued job + outbox, deterministic Bull dispatch/reconciliation, UUID-keyset batch checkpoint chỉ chứa public allowlist, measured feature progress, lease/heartbeat recovery, failure an toàn về `approved`, và final pointer switch nguyên tử/idempotent. Khi bật explicit, kể cả production, admission bắt buộc `clientIntent=desktop`; mọi default vẫn false. Local và remote exact-SHA production activation được review: GO; canonical stack đã merge. Frontend #19 tiếp tục tiêu thụ typed attachment diff trong review UI. |
...
| RBAC              | Mỗi route có allow và deny; System Admin kế thừa capability nội dung, còn Reviewer/Publisher/Editor vẫn deny-by-default ngoài capability của mình                                                                                                        |
