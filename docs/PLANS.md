...
| B-031 | Publication snapshot builder | `danangmap-backend` | B-021, B-030 | Default-off worker đã có public-only UUID-keyset batch checkpoint, deterministic checksum, feature/vertex bounds và measured monotonic progress; activation harness đã được chuẩn bị, frontend SHA `6e6fe83f7dbf6d5a01c710bb35e670e08b63e1b8` đã pin/review, còn chờ independent backend review + two-run fresh-volume evidence |
| B-032 | Atomic publish + cache invalidation | `danangmap-backend` | B-031, B-004 | Flag false giữ synchronous terminal path; flag true thử nghiệm có DB lease/heartbeat/recovery, safe failure về approved và final atomic snapshot/pointer/audit; harness được thiết kế để chứng minh SIGKILL sau batch 1/3 và resume, nhưng production vẫn fail-closed tới independent review/two-run GO |
