...
| B-031 | Publication snapshot builder | `danangmap-backend` | B-021, B-030 | Default-off worker có public-only UUID-keyset batch checkpoint, deterministic checksum, feature/vertex bounds và measured monotonic progress; local production activation exact backend `2d4675ec2385abf55fa23ad26914e037456f14cd` + frontend `6e6fe83f7dbf6d5a01c710bb35e670e08b63e1b8` đã pass independent review hai fresh-volume run; remote CI/release blockers còn mở |
| B-032 | Atomic publish + cache invalidation | `danangmap-backend` | B-031, B-004 | Flag false giữ synchronous terminal path; flag true explicit có DB lease/heartbeat/recovery, safe failure về approved và final atomic snapshot/pointer/audit. Exact production evidence chứng minh attempt 2/recovered lease 1/generation 1→2/cleanup 0/0/0; default vẫn false, PR/issues vẫn Draft/Open và release tổng thể NO-GO |
...
| F-042 | Desktop publisher preview/publish/progress | `danangmap-frontend` | F-041, F-027, B-032 | Không có action trên mobile; default sync dùng indeterminate rồi terminal; polling/reconnect/terminal failure UX. Local activation đã được chấp nhận tại exact backend `2d4675ec2385abf55fa23ad26914e037456f14cd`; remote CI còn chờ `CROSS_REPO_READ_TOKEN`. |
